...
def setLeechType(update, context):
    ...
    if user_id != int(data[1]):
        ...
        query.answer(text="Bro Now File Will Deliver As Document!", show_alert=True)
        ...
        query.answer(text="Bro Now File Will Deliver As Media!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "thumb":
        ...
        if ospath.lexists(path):
            ...
            query.answer(text="Good Boy Sucks Thumbnail Removed!", show_alert=True)
            editLeechType(message, query)
        else:
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        msg = f"Your Pic Added Bro Request By {update.message.from_user.mention_html(update.message.from_user.first_name)}."
        sendMessage(msg, context.bot, update.message)
    else:
        sendMessage("Man To Easy Like Eating Pizza 🍕 Reply To A Pic to Save Your Pic", context.bot, update.message)
